...
def process_user_complaint_message(
    ...
    """
    Saving a user's new reply and processing the complete conversation

    Flow:
    - Verify ownership
    - Confirming complaint is still draft
    - Storing user's new message
    - Invoking langgraph once
    - returning the updated complaint conversation
    """
    ...
    # Saving the user's response in database before langgraph loads the
...
def run_complaint_graph(
    ...
    """
    Running langraph for an existing saved complaint conversation

    The messages must already be stored before this function is called
    ...
    - save assistant clarification question if necessary
    """

    #By passing only IDs, PostgreSQL remains the source of truth
    complaint_graph.invoke(
        {
            'complaint_id': complaint_id,
            'user_id': user_id,
        }
    ...
    # Langgraph nodes use separate database sessions
    # clearing objects cached by the router's existing session so that
    # we reload the latest database values 
    db.expire_all()
...
def process_started_complaint(
        db: Session,
        complaint_id: int,
        user_id: int,
) -> Complaint:
    """
    Processes a newly created complaint.

    create_complaint() has already:
    - Created the Complaint row,
    - stored the first user message

    therefore, this function won't store the message again
    """
